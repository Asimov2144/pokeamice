...
**元数据（2026-09-17 补）**：24 帖统一到网页访谈的字段——`summary`（140–200 字，含具体事实）、`dek`、`original_title`、`topics`（≤5 个主题标签，词表：角色设计 / 宝可梦设计 / 音乐制作 / 地区设定 / 开发流程 / 系统设计 / 通信功能 / 传说宝可梦 / 人气投票 / 攻略…）、`entities`（people = 转写里实际发言的人，叙述体按登记表；works = 登记表的主作品；organizations 仅访谈帖给 Game Freak）、`mentions`（模型从译文里读出的其他人名与作品，不计入实体统计）、`tags` = 访谈/杂志特辑 + 机构 + 刊物 + 作品 + 人物 + 主题 + 扫描存档。由 `import-scan-set.py enrich` 生成、`normalize_meta` 归一。质量检查：逐段自动核对无漏译、无原文残留（假名只剩编辑昵称与括注）、无「〓」；抽读 B2W2 开发访谈、誕生秘話、金銀攻略本各一页，译文忠实、术语一致（ディレクター 统一为 总监）；版式上去掉了每页重复的杂志柱（ALL ABOUT…、作品名），扉页碎标题合并。
