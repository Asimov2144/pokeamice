---
layout: credits-game
title: 宝可梦冲刺赛 制作名单
game: dash
permalink: /credits/dash/
search: false
sitemap: true
gf_count: 2
---
<section class="credits__section">
<h3>Game Designers<span class="credits__ja-role">ゲームデザイン</span></h3>
<ul class="credits__names">
<li class="credits__name">Miki Obata <small>おばた みき</small></li>
<li class="credits__name">Norio Matsumura <small>松村憲男 · まつむら のりお</small></li>
</ul></section>
<section class="credits__section">
<h3>Game Concept<span class="credits__ja-role">コンセプト</span></h3>
<ul class="credits__names">
<li class="credits__name">Ein Terakawa <small>てらかわ あいん</small></li>
<li class="credits__name">Yoshihiro Hatta <small>はった よしひろ</small></li>
</ul></section>
<section class="credits__section">
<h3>Planners and Direction<span class="credits__ja-role">プランニング・ディレクション</span></h3>
<ul class="credits__names">
<li class="credits__name">Muneaki Ozawa <small>小沢宗明 · おざわ むねあき</small></li>
<li class="credits__name">Miki Obata <small>おばた みき</small></li>
<li class="credits__name">Shin Obata</li>
<li class="credits__name">Yoshihiro Hatta <small>はった よしひろ</small></li>
<li class="credits__name">Nario Matsumura</li>
<li class="credits__name">Kunimi Kawamura <small>川村久仁美 · かわむら くにみ</small></li>
</ul></section>
<section class="credits__section">
<h3>Programmers<span class="credits__ja-role">プログラム</span></h3>
<ul class="credits__names">
<li class="credits__name">Takayuki Ito <small>いとう たかゆき</small></li>
<li class="credits__name">Yosuke Suma <small>すま ようすけ</small></li>
<li class="credits__name">Nayuta Taga <small>たが なゆた</small></li>
<li class="credits__name">Ein Terakawa <small>てらかわ あいん</small></li>
<li class="credits__name">Hisato Matsumoto <small>まつもと ひさと</small></li>
</ul></section>
<section class="credits__section">
<h3>Sound<span class="credits__ja-role">サウンド</span></h3>
<ul class="credits__names">
<li class="credits__name">Miki Obata <small>おばた みき</small></li>
</ul></section>
<section class="credits__section">
<h3>Graphics<span class="credits__ja-role">グラフィック</span></h3>
<ul class="credits__names">
<li class="credits__name">Hiroki Iwasaki <small>いわさき ひろき</small></li>
<li class="credits__name">Shin Naka <small>なか しん</small></li>
<li class="credits__name">Hiroyuki Yamanaka <small>やまなか ひろゆき</small></li>
<li class="credits__name">Shin Naka <small>なか しん</small></li>
</ul></section>
<section class="credits__section">
<h3>Course Designers<span class="credits__ja-role">コースデザイン</span></h3>
<ul class="credits__names">
<li class="credits__name">Yoshiyuki Oku <small>おく よしゆき</small></li>
<li class="credits__name">Norio Matsumura <small>松村憲男 · まつむら のりお</small></li>
<li class="credits__name">Norio Matsumura <small>松村憲男 · まつむら のりお</small></li>
</ul></section>
<section class="credits__section">
<h3>English Localization</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/nakamura-hiro-noa/">Hiro Nakamura</a></li>
<li class="credits__name"><a href="/people/mcmahill-seth/">Seth McMahill</a></li>
</ul></section>
<section class="credits__section">
<h3>European Coordination</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/uesugi-hiroyuki/">Hiroyuki Uesugi</a></li>
</ul></section>
<section class="credits__section">
<h3>English Translation</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/nakamura-hiro-noa/">Hiro Nakamura</a></li>
</ul></section>
<section class="credits__section">
<h3>French Translation</h3>
<ul class="credits__names">
<li class="credits__name">Sabrina Bretant</li>
</ul></section>
<section class="credits__section">
<h3>German Translation</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/schafers-daniel/">Daniel Schäfers</a></li>
</ul></section>
<section class="credits__section">
<h3>Italian Translation</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/nardo-elena/">Elena Nardo</a></li>
</ul></section>
<section class="credits__section">
<h3>Spanish Translation</h3>
<ul class="credits__names">
<li class="credits__name">Palma Sánchez de Maya</li>
</ul></section>
<section class="credits__section">
<h3>Debug</h3>
<ul class="credits__names">
<li class="credits__name is-company">Super Mario Club</li>
<li class="credits__name is-company">NOA Product Testing</li>
<li class="credits__name">Robert Johnson</li>
<li class="credits__name">Patrick Taylor</li>
<li class="credits__name"><a href="/people/lillygren-teresa/">Teresa Lillygren</a></li>
<li class="credits__name">Mika Kurosawa</li>
<li class="credits__name is-company">NOE Product Testing</li>
<li class="credits__name">Alessio Danieli</li>
<li class="credits__name">Daniel Barsan</li>
<li class="credits__name">Steffen Lotz</li>
<li class="credits__name">Stefania Montagnese</li>
<li class="credits__name">Arrate Ugarte</li>
</ul></section>
<section class="credits__section">
<h3>Package Illustration<span class="credits__ja-role">パッケージイラスト</span></h3>
<ul class="credits__names">
<li class="credits__name is-gf"><a href="/people/sugimori-ken/">Ken Sugimori</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 18 部 GF 自研作品里有开发职务署名，最早 1996）"> <small>杉森建 · すぎもり けん</small></li>
</ul></section>
<section class="credits__section">
<h3>Artwork<span class="credits__ja-role">アートワーク</span></h3>
<ul class="credits__names">
<li class="credits__name">Sakae Kimura <small>きむら さかえ</small></li>
<li class="credits__name">Chiaki Sinkai <small>しんかい ちあき</small></li>
<li class="credits__name"><a href="/people/nakamichi-sachiko/">Sachiko Nakamichi</a> <small>なかみち さちこ</small></li>
<li class="credits__name">Fujiko Nomura <small>のむら ふじこ</small></li>
</ul></section>
<section class="credits__section">
<h3>Pokémon Voice (PIKACHU)<span class="credits__ja-role">こえのしゅつえん(ピカチュウ)</span></h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/otani-ikue/">Ikue Ohtani</a> <small>大谷育江 · おおたに いくえ</small></li>
</ul></section>
<section class="credits__section">
<h3>Development Support<span class="credits__ja-role">かいはつ・サポート</span></h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/miura-masayuki/">Masayuki Miura</a> <small>三浦昌幸 · みうら まさゆき</small></li>
<li class="credits__name">Hizuki Misono <small>みその ひづき</small></li>
<li class="credits__name"><a href="/people/onishi-takahiro/">Takahiro Onishi</a> <small>おおにし たかひろ</small></li>
<li class="credits__name">Takao Nakano <small>なかの たかお</small></li>
</ul></section>
<section class="credits__section">
<h3>Special Thanks<span class="credits__ja-role">スペシャルサンクス</span></h3>
<ul class="credits__names">
<li class="credits__name is-gf"><a href="/people/sugimori-ken/">Ken Sugimori</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 18 部 GF 自研作品里有开发职务署名，最早 1996）"> <small>杉森建 · すぎもり けん</small></li>
<li class="credits__name is-gf"><a href="/people/toyama-kenchi/">Kenkichi Toyama</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 8 部 GF 自研作品里有开发职务署名，最早 2004）"> <small>とやま けんきち</small></li>
<li class="credits__name is-gf"><a href="/people/masuda-junichi/">Junichi Masuda</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 20 部 GF 自研作品里有开发职务署名，最早 1996）"> <small>増田順一 · ますだ じゅんいち</small></li>
<li class="credits__name">Masato Ishii <small>いしい まさと</small></li>
<li class="credits__name is-gf"><a href="/people/anazawa-masamichi/">Masamichi Anazawa</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 4 部 GF 自研作品里有开发职务署名，最早 2013）"> <small>あなざわ まさみち</small></li>
<li class="credits__name is-gf"><a href="/people/mitsuhara-satoshi/">Satoshi Mitsuhara</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 4 部 GF 自研作品里有开发职务署名，最早 2004）"> <small>みつはら さとし</small></li>
<li class="credits__name">Shuichi Narusawa <small>なるさわ しゅういち</small></li>
<li class="credits__name">Toshifumi Sayama</li>
<li class="credits__name">Adrian Michalski</li>
</ul></section>
<section class="credits__section">
<h3>Pokémon Original Director<span class="credits__ja-role">ポケットモンスター・オリジナルディレクター</span></h3>
<ul class="credits__names">
<li class="credits__name is-gf"><a href="/people/tajiri-satoshi/">Satoshi Tajiri</a><img class="credits__gf" src="/assets/img/works/gamefreak-mark.svg" alt="Game Freak" title="Game Freak 成员（按名单推断：在 9 部 GF 自研作品里有开发职务署名，最早 1996）"> <small>田尻智 · たじり さとし</small></li>
</ul></section>
<section class="credits__section">
<h3>Producers<span class="credits__ja-role">プロデューサー</span></h3>
<ul class="credits__names">
<li class="credits__name">Kunimi Kawamura <small>川村久仁美 · かわむら くにみ</small></li>
<li class="credits__name"><a href="/people/yamagami-hitoshi/">Hitoshi Yamagami</a> <small>山上仁志 · やまがみ ひとし</small></li>
</ul></section>
<section class="credits__section">
<h3>Executive Producer<span class="credits__ja-role">エグゼクティブ・プロデューサー</span></h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/iwata-satoru/">Satoru Iwata</a> <small>岩田聡 · いわた さとる</small></li>
</ul></section>
<section class="credits__section">
<h3>Executive Producer</h3>
<ul class="credits__names">
<li class="credits__name"><a href="/people/ishihara-tsunekazu/">Tsunekaz Ishihara</a></li>
</ul></section>
